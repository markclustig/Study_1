...
conditions <- list(
  ...
  n_cases = c(100, 200, 500, 1000), # number of simulated participants
...
simulate <- function(n_rep, n_cases, res_var, res_cor, lat_cov, int, slope,
  ...
  dat <- mvrnorm(n = n_cases, mu = E_y, Sigma = sigma) %>%
    as.data.frame()
  names(dat) <- paste0("t", seq(n_rep))
...
analyze_mi <- function(mi_list, mod_autocor, mod_no_autocor) {
  ...
  noaut_results <- mod_noaut_result %>%
    ...
    rownames_to_column() %>%
    extract_results() %>%
    rename_with(~ paste0("mod_noaut_", .x))
